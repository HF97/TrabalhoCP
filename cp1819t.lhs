...
calcula :: Expr -> Int
calcula = cataExpr (either id (Cp.ap . (strOp >< id)))
    where
        strOp (Op "+") = uncurry (+)
        strOp (Op "-") = uncurry (-)
        strOp (Op "*") = uncurry (*)
        strOp (Op "/") = uncurry (div)
...
show' = cataExpr (either l r)
    where
        l = show
        r = (("( " ++) . (++ " )")) . uncurry (++) . (uncurry (++) >< id) . ((id >< opStr2) >< id) . (swap >< id) . assocl
...
pushInt :: Int -> String
pushInt i = "PUSH " ++ show i



exprCod :: Expr -> Codigo
exprCod = cataExpr (either l r)
    where
        l = singl . pushInt
        r = (uncurry (++)) . ((uncurry (++)) >< (singl . opStr)) . swap
...
dimen = cataL2D (either ((fromIntegral >< fromIntegral) . p1) dimen2)
    where
      dimen2 (V,((wa,ha),(wb,hb))) = (wa/2 + (max (wa/2) wb), ha+hb)
      dimen2 (Ve,((wa,ha),(wb,hb))) = (max wa wb, ha+hb)
      dimen2 (Vd,((wa,ha),(wb,hb))) = (wa+wb, ha+hb)
      dimen2 (H,((wa,ha),(wb,hb)))  = (wa+wb, ha/2 + (max (ha/2) hb))
      dimen2 (Hb,((wa,ha),(wb,hb))) = (wa+wb, max ha hb)
      dimen2 (Ht,((wa,ha),(wb,hb))) = (wa+wb, ha+hb)

calcOrigins :: ((X Caixa Tipo),Origem) -> X (Caixa,Origem) ()
calcOrigins (Unid a,o) = Unid (a,o)
calcOrigins ((Comp t l r),o@(x,y)) = Comp () (calcOrigins(l,o)) (calcOrigins(r,calc t o (dimen l)))
              

agrup_caixas :: X (Caixa, Origem) () -> Fig
agrup_caixas (Unid a) = singl(swap a)
agrup_caixas (Comp t l r) = (agrup_caixas l) ++ (agrup_caixas r)
...
s 0 = 12
s (n+1) =  linear (n+1) + s n

linear 1 = 18
linear (n+1) = 8 + linear (n)

cos' x = prj . for b i 
  where 
      i = (1 , -((x*x)/2),12,18)
      b(c,h,s,linear) = (h + c, -((x*x)/s) * h, linear + s, 8 + linear)
      prj(c,h,s,linear) = c
...
tar :: FS a b -> [(Path a, b)]
tar = cataFS (concat . map ((either (singl . (singl >< id)) (tar2)) . distr))
    where
      tar2 (a, []) = []
      tar2 (a, (p,b):xs) = (a:p,b):(tar2 (a,xs))

untar :: (Eq a) => [(Path a, b)] -> FS a b
untar = joinDupDirs . cataList(either untar2 addF)
  where
    untar2 () = FS []
    addF (([x],b), FS l) = FS ((x, File b):l)
    addF (((h:t), b), FS l) = FS (singl(h, Dir (addF ((t,b), FS l))))
